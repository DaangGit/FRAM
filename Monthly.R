...
titan <- getSymbols.yahoo("TITAN.NS", from = "2020-04-01", to = "2022-03-31", verbose = F, auto.assign = F, periodicity = "monthly")
titan<- na.omit(titan)

torntpower <- getSymbols.yahoo("TORNTPOWER.NS", from ="2020-04-01", to = "2022-03-31", verbose = F, auto.assign = F, periodicity = "monthly")
torntpower <- na.omit(torntpower)

# T-Bill Data
D_W_M_T_Bills <- read_excel("M-T-Bills.xlsx")
...
extitan <- as.data.frame(monthlyReturn(titan$TITAN.NS.Close)[-1, ] - D_W_M_T_Bills$`T-Bill% (Monthly)`)
extorntpower <- as.data.frame(monthlyReturn(torntpower$TORNTPOWER.NS.Close)[-1, ] - D_W_M_T_Bills$`T-Bill% (Monthly)`)
...
# Return Calculation
returns_titan <- as.xts(tail(data.frame(titan$TITAN.NS.Close),-1)/head(data.frame(titan$TITAN.NS.Close),-1)-1, frequency = 12)
returns_titan <- na.omit(returns_titan)
returns_torntpower <- as.xts(tail(data.frame(torntpower$TORNTPOWER.NS.Close),-1)/head(data.frame( torntpower$TORNTPOWER.NS.Close),-1)-1, frequency = 12)
...
# Data Collection
titan1 <- getSymbols("TITAN.NS", from = "2020-04-01", to = "2022-03-31", periodicity = "monthly")
head(TITAN.NS)

torntpower1 <- getSymbols("TORNTPOWER.NS", from ="2020-04-01", to = "2022-03-31", periodicity = "monthly") 
head(TORNTPOWER.NS)
