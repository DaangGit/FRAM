...
titan <- getSymbols.yahoo("TITAN.NS", from = "2020-04-01", to = "2022-03-31", verbose = F, auto.assign = F)
titan<- na.omit(titan)
titan <- titan[index(titan) != '2020-11-14']

torntpower <- getSymbols.yahoo("TORNTPOWER.NS", from ="2020-04-01", to = "2022-03-31", verbose = F, auto.assign = F)
torntpower <- na.omit(torntpower)
torntpower <- torntpower[index(torntpower) != '2020-11-14']

# T-Bill Data
D_W_M_T_Bills <- read_excel("D-T-Bills.xlsx")
D_W_M_T_Bills <- as.data.frame(D_W_M_T_Bills)
D_W_M_T_Billsxts <- xts(D_W_M_T_Bills[,-1], order.by = as.Date(D_W_M_T_Bills$Date))

# Excess Returns
exNSE <- as.data.frame(dailyReturn(NSE$NSEI.Close)[-1, ] - D_W_M_T_Bills$`T-Bill% (Daily)`)
extitan <- as.data.frame(dailyReturn(titan$TITAN.NS.Close)[-1, ] - D_W_M_T_Bills$`T-Bill% (Daily)`)
extorntpower <- as.data.frame(dailyReturn(torntpower$TORNTPOWER.NS.Close)[-1, ] - D_W_M_T_Bills$`T-Bill% (Daily)`)
...
# Return Calculation
returns_titan <- as.xts(tail(data.frame(titan$TITAN.NS.Close),-1)/head(data.frame(titan$TITAN.NS.Close),-1)-1, frequency = 365)
returns_titan <- na.omit(returns_titan)
returns_torntpower <- as.xts(tail(data.frame(torntpower$TORNTPOWER.NS.Close),-1)/head(data.frame( torntpower$TORNTPOWER.NS.Close),-1)-1, frequency = 365)
...
# Torrent Power
plot(acf(returns_torntpower , lag.max = 10))
plot(pacf(returns_torntpower , lag.max = 10))
...
# Data Collection
titan1 <- getSymbols("TITAN.NS", from = "2020-04-01", to = "2022-03-31")
head(TITAN.NS)

torntpower1 <- getSymbols("TORNTPOWER.NS", from ="2020-04-01", to = "2022-03-31") 
head(TORNTPOWER.NS)
